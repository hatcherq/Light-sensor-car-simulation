...
int main(int argc, char *argv[]) {
    ...
    SDL_Event event;
    while(!done){
        SDL_SetRenderDrawColor(renderer, 0, 157, 196, 255);
        SDL_RenderClear(renderer);

        //Move robot based on user input commands/auto commands
        if (robot.auto_mode == 1)
        robotAutoMotorMove(&robot, front_centre_sensor, left_sensor, right_sensor, renderer);
        ...
        while(SDL_PollEvent(&event)){
            ...
            if(state[SDL_SCANCODE_SPACE]){
                setup_robot(&robot);
                crashed = 0; // Reset the crashed state
                front_centre_sensor = 0; // Reset sensors if needed
                left_sensor = 0;
                right_sensor = 0;
            }
